...
public class PayServiceImpl implements PayService {
    ...
    public boolean payCallback(HttpServletRequest request) throws BusinessException {
        ...
        if (DealParamStr.verifySignature(resMap)) {
            ...
            String state = resMap.get(ParamKey.PAY_STATUS.getParamKey());

            // 判断支付状态
            // 如果反馈的支付状态为"预登记"，则记录已受理
            // 如果反馈的支付状态为"支付成功"或"支付关闭"，则记录交易成功
            // 如果反馈的支付状态为其他结果，则记录交易失败
            if (state.equals(PayStatus.RECEIVED.getPayStatusCode())) {
                billBean.setTradeState(TradeState.ACCEPTSUCCESS.getTradeState());
            } else if (state.equals(PayStatus.TRADE_SUCCESS.getPayStatusCode()) || state.equals(PayStatus.TRADE_CLOSE.getPayStatusCode())) {
